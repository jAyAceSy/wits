-- =====================================================================
-- WITS — Migration 006: Finisher / QC columns for pallet labels
--
-- Adds two more optional columns to Production's Transfer Excel upload
-- (alongside Destination Warehouse / Production Date / Pallet Number
-- from migration_005), carried through to transfer_master and printed
-- on the pallet label.
--
-- Run this AFTER migration_005_unify_label_printing.sql.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Updated import: passes finisher/qc straight through, no validation
-- (they're purely informational, not required to import or to print).
-- ---------------------------------------------------------------------
...
begin
  ...
  insert into public.transfer_master
    (transfer_barcode, item_code, description, uom, transferred_quantity, status, import_batch_id,
     destination_warehouse, production_date, pallet_number, finisher, qc)
  select s.transfer_barcode, s.item_code, s.description, s.uom, s.transferred_quantity, 'Pending', p_batch_id,
         ...
         nullif(trim(s.finisher), ''), nullif(trim(s.qc), '')
